        ...
        <div className="flex items-center">
          <div className="shrink-0">
            <img
              src={displayImageUrl}
              alt={item.title}
              className="max-w-full h-auto rounded border border-slate-200 dark:border-slate-700 transition-colors"
          ...
          <div className="grow ml-4">
            <h6 className="mb-1 truncate text-slate-900 dark:text-white transition-colors" style={{ maxWidth: '250px' }}>{item.title}</h6>
            <div className="mb-2">
              <span className="text-orange-500 dark:text-orange-400 font-bold text-lg mr-2 transition-colors">₹{item.discountPrice.toFixed(2)}</span>
              {item.originalPrice > item.discountPrice && (
                <span className="text-slate-500 dark:text-slate-400 line-through small transition-colors">
                  ₹{item.originalPrice.toFixed(2)}
                </span>
              )}
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center border border-slate-200 dark:border-slate-700 rounded-full px-2 py-1 bg-white dark:bg-slate-800 shadow-sm transition-colors" style={{ width: '110px' }}>
                <button
                  className="btn btn-sm text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full flex items-center justify-center border-0 p-0 transition-colors"
                ...
                <span className="mx-auto font-bold text-slate-900 dark:text-white transition-colors">{item.quantity}</span>
                <button
                  className="btn btn-sm text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full flex items-center justify-center border-0 p-0 transition-colors"
                  style={{ width: '28px', height: '28px' }}
                  onClick={() => onUpdateQuantity(item.id, item.quantity + 1)}
