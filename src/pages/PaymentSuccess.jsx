...
const PaymentSuccess = () => {
    ...
    return (
      <div className="container py-12 text-center min-h-screen flex flex-col items-center justify-center">
        <div className="flex flex-col relative min-w-0 break-words shadow-lg border-0 bg-white dark:bg-slate-900 transition-colors p-12 rounded-2xl payment-empty-card">
          ...
          <h1 className="font-bold text-slate-900 dark:text-white mb-6">No Recent Purchases Found</h1>
          <p className="text-slate-600 dark:text-slate-400 mb-6 text-lg">We couldn't find a recent order associated with this session.</p>
          <div className="flex justify-center gap-3">
            <button onClick={() => navigate('/')} className="btn btn-primary rounded-full px-6 py-2 font-medium shadow-sm">Go Home</button>
            <Link to="/deals" className="btn btn-outline-secondary rounded-full px-6 py-2 font-medium dark:text-slate-300 dark:border-slate-600">Browse Deals</Link>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="container py-12 text-center min-h-screen flex flex-col items-center justify-center payment-page-container">
      {/* Confetti */}
      {showConfetti && (
      ...
      <div className="flex flex-col relative min-w-0 break-words shadow-lg border-0 bg-white dark:bg-slate-900 transition-colors p-12 rounded-2xl payment-card">
        ...
        <h1 className="font-bold text-slate-900 dark:text-white mb-2">Payment Successful! 🎉</h1>
        <p className="text-slate-600 dark:text-slate-400 mb-6 text-lg">Thank you for your purchase. Your order has been placed successfully.</p>
        ...
        <div className="bg-slate-50 dark:bg-slate-800 rounded-2xl p-6 mb-6 text-left transition-colors">
          <div className="flex justify-between mb-2">
            ...
            <span className="font-bold text-slate-900 dark:text-white">#{state.orderId}</span>
          </div>
        ...
        {state.deals && state.deals.length > 0 && (
          <div className="mb-6 text-left">
            <h5 className="font-bold text-slate-900 dark:text-white mb-6">Order Summary</h5>
                        ...
                        <div className="flex items-center">
                          {deal.imageUrl ? (
                            ...
                            <img
                              ...
                              alt={deal.title}
                              className="rounded mr-2 order-item-thumb"
                            />
                          ) : (
                            <div className="rounded mr-2 bg-slate-200 dark:bg-slate-600 flex justify-center items-center order-item-placeholder">
                              <span>📦</span>
                            </div>
                          )}
                          <span className="font-semibold text-slate-900 dark:text-white">{deal.title || 'Software Deal'}</span>
                        </div>
                      </td>
                      <td>₹{(deal.discountPrice || 0).toFixed(2)}</td>
                      <td>{deal.quantity}</td>
                      <td className="text-right font-bold">₹{((deal.discountPrice || 0) * deal.quantity).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr>
                    ...
                    <td className="text-right font-bold text-orange-500 text-lg">₹{(state.amount || 0).toFixed(2)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
        ...
        {/* Share flex flex-wrap -mx-6 */}
          ...
          <span className="text-slate-500 dark:text-slate-400 font-medium text-sm">Share this deal:</span>
          <button
            className="btn btn-sm flex items-center gap-2 rounded-full px-6 font-bold border-0 share-whatsapp"
        ...
        <div className="flex gap-3 justify-center">
          <Link to="/orders" className="btn btn-primary px-6 py-2 font-medium shadow-sm rounded-full">
