...
function DealDetails() {
      ...
      if (user) {
        try {
          ...
          const matchingOrder = ordersData.find(
            (o) =>
              o.dealId === Number(id) && o.status.toLowerCase() === "completed",
          );
          if (matchingOrder) {
  ...
  const handlePurchase = async () => {
    ...
    if (!user) {
      ...
      navigate("/login", {
        state: { returnTo: `/deals/${deal.id}`, buyNowItem },
      });
      return;
    }
  ...
  const savingsPercent =
    deal.originalPrice > 0
      ? Math.round(
          ((deal.originalPrice - deal.discountPrice) / deal.originalPrice) *
            100,
        )
      : 0;
  ...
  return (
    <div className="deal-details-page py-12">
      ...
      <Link
        to="/deals"
        className="no-underline font-bold text-slate-500 dark:text-slate-400 hover:text-orange-500 dark:hover:text-orange-400 pl-0 mb-6 transition-colors"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8">
          ...
          <div className="mb-6">
            ...
            <h1 className="font-extrabold text-slate-900 dark:text-white tracking-tight transition-colors">
              {deal.title}
            </h1>
          ...
          {/* ADMIN ONLY CONTROLS */}
          {isAdmin && (
            <div className="flex flex-col relative min-w-0 break-words border-blue-500 p-6 rounded-2xl bg-blue-50 dark:bg-blue-900/20 shadow-sm my-12">
              <h4 className="font-bold text-blue-600 mb-6">
              ...
              <div className="grid md:grid-cols-12 gap-6 items-end mb-6">
                ...
                <div className="md:col-span-8">
                  ...
                  >
                    ...
                    <button
                      type="submit"
                      className="mt-6 py-2 px-6 font-bold self-end"
                      disabled={uploadingImage}
                    >
                ...
                {/* Actions Section */}
                <div className="md:col-span-4 flex justify-end gap-2 md:text-right md:justify-end">
                  <button
                    onClick={openEditForm}
                    className="py-2 px-6 font-bold rounded-full shadow-sm text-slate-900"
                  >
                    ✏️ Edit Deal
                  </button>
                  <button
            ...
            {activeTab === "Overview" && (
              <div className="animation-fade-in">
                <div className="deal-full-description mb-12">
                  <h3 className="font-bold text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-800 pb-2 mb-6 transition-colors">
                    About this software
                  </h3>
                  <p
                    className="text-slate-500 dark:text-slate-400 text-base transition-colors"
                    style={{ whiteSpace: "pre-line", lineHeight: "1.7" }}
                  >
                    {deal.description ||
                      "No full description provided for this software package. Check features below for more info."}
                  </p>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {featureList.map((feature, idx) => (
                      <div key={idx} className="flex items-center gap-3">
                        <div
                          className="bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400 rounded-full flex items-center justify-center mr-4 transition-colors"
                          style={{ width: "24px", height: "24px" }}
                        >
                          <span className="font-bold text-sm">✓</span>
                        </div>
                        <span className="text-slate-900 dark:text-slate-100 font-medium text-base transition-colors">
                          {feature}
                        </span>
                      </div>
                    ))}
                  ...
                  <div className="deal-terms mb-12">
                    ...
                    <ul className="space-y-2 border-0">
                      {termList.map((term, idx) => (
                        <li
                          key={idx}
                          className="bg-transparent text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 px-0 py-2 text-base transition-colors"
                        >
                          • {term}
                        </li>
                      ))}
                      <li className="bg-transparent text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 px-0 py-2 text-base transition-colors">
                ...
                <ul className="list-group list-group-flush border-0 bg-transparent mb-6">
                  {(deal.shortDescription || "")
                    ...
                    .map((feature, idx) => (
                      <li
                        key={idx}
                        className="list-group-item bg-transparent border-0 flex items-start text-slate-600 dark:text-slate-300 py-2 text-base"
                      >
                        <span className="text-orange-500 mr-4 mt-1 font-bold">
                          ✓
                        </span>{" "}
                        {feature}
                      </li>
            ...
            {activeTab === "Reviews" && (
              <div className="animation-fade-in">
                ...
                <div className="reviews-section mt-12 pt-6 border-t border-slate-200 dark:border-slate-800 transition-colors">
                  <h3 className="font-bold text-slate-900 dark:text-white mb-6 transition-colors">
                    Customer Reviews
                  </h3>
                  ...
                  <div className="flex items-center mb-6 p-6 rounded-2xl card-shadow bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 transition-colors">
                    <div className="text-center mr-6 pr-6 border-r border-slate-200 dark:border-slate-800 transition-colors">
                      ...
                      <div className="text-orange-500 text-lg mb-1 flex justify-center">
                        {renderStars(calculatedAverageRating)}
                      </div>
                      <span className="text-slate-500 dark:text-slate-400 font-bold text-base transition-colors">
                        Based on {calculatedReviewCount || 0} reviews
                      </span>
                    </div>
                    <div>
                      <h5 className="font-bold text-slate-900 dark:text-white mb-1 transition-colors">
                        Share your thoughts
                      </h5>
                      <p className="text-slate-500 dark:text-slate-400 text-sm mb-0 transition-colors">
                        Only verified purchasers of this deal can write a
                        review.
                      </p>
                    </div>
                  </div>
                  ...
                  {user ? (
                    hasPurchased ? (
                      <div className="flex flex-col relative min-w-0 break-words p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 mb-12 transition-colors">
                        ...
                        <form onSubmit={handleReviewSubmit}>
                          <div className="mb-6">
                            <label className="font-bold text-sm text-slate-900 dark:text-white transition-colors mb-2 block">
                              Rating
                            </label>
                            <div
                          ...
                          <div className="mb-6">
                            <label className="font-bold text-sm text-slate-900 dark:text-white transition-colors">
                              Comment
                            </label>
                            <textarea
                              className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white border-slate-200 dark:border-slate-700 transition-colors"
                              rows="4"
                      ...
                      <div className="relative px-4 border border-transparent py-6 mb-12 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 border-blue-200 dark:border-blue-800 transition-colors">
                        ℹ️ Only customers who purchased this deal can write a
                        review.
                        <Link
                          to="/deals"
                          className="font-bold ml-1 no-underline text-orange-500 dark:text-orange-400"
                        >
                          Explore deals
                        </Link>{" "}
                        to buy.
                    ...
                    <div className="relative px-4 border border-transparent py-6 mb-12 rounded-lg bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-300 border-amber-200 dark:border-amber-800 transition-colors">
                      ...
                      <Link
                        to="/login"
                        className="font-bold no-underline text-orange-500 dark:text-orange-400"
                      >
                        log in
                      </Link>{" "}
                      to write a review.
                  ...
                  {loadingReviews ? (
                    <div className="text-center py-6">
                      <span
                        className="inline-block w-4 h-4 border-2 border-current border-r-transparent rounded-full animate-spin text-orange-500"
                        role="status"
                      ></span>
                      <span className="ml-2 text-slate-500 dark:text-slate-400 transition-colors">
                        Loading reviews...
                      </span>
                    </div>
                  ) : reviews.length === 0 ? (
                    <div className="text-center py-12 rounded-2xl shadow-sm bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 transition-colors empty-state-container">
                      ...
                      <p className="text-slate-500 dark:text-slate-400 mb-0 transition-colors">
                        Be the first reviewer to share your experience.
                      </p>
                    </div>
                  ) : (
                    <div className="reviews-list-container">
                      <div className="flex justify-between items-center mb-6">
                        <h5 className="font-bold text-slate-900 dark:text-white mb-0">
                          All Reviews
                        </h5>
                        <div className="flex items-center">
                          <label className="mr-2 font-bold text-slate-600 dark:text-slate-300 text-sm">
                            Sort Reviews
                          </label>
                          <select
                      ...
                      <div className="reviews-list flex flex-col gap-4">
                        {sortedReviews.map((r) => (
                          <div
                            key={r.id}
                            className="review-card p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 transition-colors"
                          >
                            <div className="flex justify-between items-center mb-2">
                              <div className="flex items-center gap-2">
                                ...
                                {r.role === "Admin" ? (
                                  <span className="inline-block leading-none text-center whitespace-nowrap align-baseline rounded bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400 text-xs px-2 py-1">
                                    Admin
                                  </span>
                                ) : (
                                  <span className="inline-block leading-none text-center whitespace-nowrap align-baseline rounded bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400 text-xs px-2 py-1">
                                    Verified Buyer
                                  </span>
                                )}
                              </div>
                            ...
                            <div className="text-orange-500 mb-2 text-base">
                              {renderStars(r.rating)}
                            </div>
                            <p
                              className="text-slate-500 dark:text-slate-400 text-base mb-0 transition-colors"
        ...
        <div className="lg:col-span-4">
          <div className="sticky top-24 self-start">
            <div className="deal-purchase-card flex flex-col relative min-w-0 break-words border border-slate-100 dark:border-slate-800 card-shadow p-6 rounded-2xl bg-white dark:bg-slate-900 mb-6 transition-colors">
              <h5 className="uppercase text-slate-500 dark:text-slate-400 font-bold text-xs tracking-wider mb-2 transition-colors">
                Lifetime Deal Offer
              </h5>
              <h3 className="font-extrabold text-slate-900 dark:text-white mb-6 transition-colors">
                {deal.title}
              </h3>
                ...
                {calculatedReviewCount > 0 ? (
                  <>
                    ...
                    <span className="font-bold text-slate-900 dark:text-white text-sm transition-colors">
                      {calculatedAverageRating?.toFixed(1) || "0.0"}/5 (
                      {calculatedReviewCount} Review
                      {calculatedReviewCount !== 1 ? "s" : ""})
                    </span>
                  </>
                ) : (
                  <span className="text-slate-500 dark:text-slate-400 text-sm transition-colors">
              ...
              <div className="mb-6">
                {deal.stock > 0 ? (
                  <>
                    ...
                    {/* Quantity selector */}
                    <label className="font-bold text-slate-900 dark:text-white text-sm mb-2 transition-colors">
                      Select Quantity
                    </label>
                    <select
                      className="
                    ...
                    >
                      {[...Array(Math.min(10, deal.stock))].map((_, i) => (
                        <option key={i + 1} value={i + 1}>
                          {i + 1} code{i > 0 ? "s" : ""} (₹
                          {(deal.discountPrice * (i + 1)).toFixed(2)})
                        </option>
                      ))}
                    </select>
      ...
      <div
        className={`fixed bottom-0 end-0 p-6 hidden lg:block transition-all`}
        style={{
          zIndex: 1000,
          transform: showStickySidebar ? "translateY(0)" : "translateY(150%)",
          right: "2rem",
          bottom: "2rem",
        }}
      >
